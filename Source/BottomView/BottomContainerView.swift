...
open class BottomContainerView: UIView {
  ...
  lazy var pickerButton: ButtonPicker = { [unowned self] in
    let pickerButton = ButtonPicker(configuration: self.pickerConfiguration)
    pickerButton.setTitleColor(UIColor.white, for: UIControl.State())
    pickerButton.delegate = self
    pickerButton.numberLabel.isHidden = !self.pickerConfiguration.showsImageCountLabel
  ...
  open lazy var doneButton: UIButton = { [unowned self] in
    let button = UIButton()
    button.setTitle(self.pickerConfiguration.cancelButtonTitle, for: UIControl.State())
