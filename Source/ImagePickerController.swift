...
open class ImagePickerController: UIViewController {
  ...
  open lazy var galleryView: ImageGalleryView = { [unowned self] in
    let galleryView = ImageGalleryView(configuration: self.pickerConfiguration)
  ...
  open lazy var bottomContainer: BottomContainerView = { [unowned self] in
    let view = BottomContainerView(configuration: self.pickerConfiguration)
    view.backgroundColor = self.pickerConfiguration.bottomContainerColor
    view.delegate = self
  ...
  open lazy var topView: TopView = { [unowned self] in
    let view = TopView(configuration: self.pickerConfiguration)
  ...
  lazy var cameraController: CameraView = { [unowned self] in
    let controller = CameraView(configuration: self.pickerConfiguration)
  ...
  @objc public required init(configuration: PickerConfiguration = PickerConfiguration()) {
  ...
  open override func viewWillAppear(_ animated: Bool) {
    ...
    if pickerConfiguration.managesAudioSession {
  ...
  func presentAskPermissionAlert() {
    let alertController = UIAlertController(title: pickerConfiguration.requestPermissionTitle, message: pickerConfiguration.requestPermissionMessage, preferredStyle: .alert)

    let alertAction = UIAlertAction(title: pickerConfiguration.OKButtonTitle, style: .default) { _ in
      if let settingsURL = URL(string: UIApplication.openSettingsURLString) {
        UIApplication.shared.openURL(settingsURL)
      }
    }

    let cancelAction = UIAlertAction(title: pickerConfiguration.cancelButtonTitle, style: .cancel) { _ in
  ...
  @objc func volumeChanged(_ notification: Notification) {
    guard pickerConfiguration.allowVolumeButtonsToTakePicture,
  ...
  func enableGestures(_ enabled: Bool) {
    ...
    topView.rotateCamera.isEnabled = pickerConfiguration.canRotateCamera
  }
  ...
  fileprivate func takePicture() {
    ...
    if pickerConfiguration.collapseCollectionViewWhileShot {
...
extension ImagePickerController: CameraViewDelegate {
  ...
  func setFlashButtonHidden(_ hidden: Bool) {
    if pickerConfiguration.flashButtonAlwaysHidden {
  ...
  func imageToLibrary() {
    ...
    galleryView.fetchPhotos {
      ...
      if self.pickerConfiguration.allowMultiplePhotoSelection == false {
  ...
  func applyOrientationTransforms() {
    let rotate = pickerConfiguration.rotationTransform
