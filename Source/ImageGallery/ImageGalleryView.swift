...
open class ImageGalleryView: UIView {
  ...
  lazy var collectionViewLayout: UICollectionViewLayout = { [unowned self] in
    let layout = ImageGalleryLayout(configuration: self.pickerConfiguration)
  ...
  lazy var topSeparator: UIView = { [unowned self] in
    ...
    view.backgroundColor = self.pickerConfiguration.gallerySeparatorColor
  ...
  public init(configuration: PickerConfiguration? = nil) {
  ...
  func configure() {
    ...
    topSeparator.addSubview(pickerConfiguration.indicatorView)
  ...
  func updateFrames() {
    ...
    topSeparator.autoresizingMask = [.flexibleLeftMargin, .flexibleRightMargin, .flexibleWidth]
      pickerConfiguration.indicatorView.frame = CGRect(x: (totalWidth - pickerConfiguration.indicatorWidth) / 2, y: (topSeparator.frame.height - pickerConfiguration.indicatorHeight) / 2,
      width: pickerConfiguration.indicatorWidth, height: pickerConfiguration.indicatorHeight)
  ...
  func fetchPhotos(_ completion: (() -> Void)? = nil) {
    AssetManager.fetch(withConfiguration: pickerConfiguration) { assets in
...
extension ImageGalleryView: UICollectionViewDelegate {
  ...
  public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    ...
    if pickerConfiguration.allowMultiplePhotoSelection == false {
    ...
    AssetManager.resolveAsset(asset, size: CGSize(width: 100, height: 100), shouldPreferLowRes: pickerConfiguration.useLowResolutionPreviewImage) { image in
