...
class CameraView: UIViewController, CLLocationManagerDelegate, CameraManDelegate {
  ...
  lazy var noCameraButton: UIButton = { [unowned self] in
    ...
    let title = NSAttributedString(string: self.pickerConfiguration.settingsTitle,
      attributes: [
        NSAttributedString.Key.font: self.pickerConfiguration.settingsFont,
        NSAttributedString.Key.foregroundColor: self.pickerConfiguration.settingsColor
      ])

    button.setAttributedTitle(title, for: UIControl.State())
    button.contentEdgeInsets = UIEdgeInsets(top: 5.0, left: 10.0, bottom: 5.0, right: 10.0)
    button.sizeToFit()
    button.layer.borderColor = self.pickerConfiguration.settingsColor.cgColor
  ...
  override func viewDidLoad() {
    ...
    if pickerConfiguration.recordLocation {
    ...
    if pickerConfiguration.allowPinchToZoom {
  ...
  func setupPreviewLayer() {
    ...
    layer.backgroundColor = pickerConfiguration.mainColor.cgColor
